...
/*Wartość x +/- p% można interpretować jako przedział [x - (x * p / 100), x + (x * p / 100)]*/
typedef struct wartosc 
{
  double lower; // dolne ograniczenie przedziału
  double upper; // górne ograniczenie przedziału
  bool disjoint; // jeśli przedział jest typu (-HUGE_VAL, x] u [y, HUGE_VAL], to będzie reprezentowany jako [x, y] z flagą disjoint ustawioną na true
} wartosc;
