...
![Transformer](https://raw.githubusercontent.com/MakuwaDev/UW-Projects/main/Year3/Deep%20Neural%20Networks/Task3/transformer.png)
...
![Architecture](https://raw.githubusercontent.com/MakuwaDev/UW-Projects/main/Year3/Deep%20Neural%20Networks/Task3/architecture.png)
...
$$W_Q, W_K, W_V \in \mathbb{R}^{\text{hid dim} \times (\text{num head} \cdot \text{head dim})}$$
...
$$A' = QK^T \quad \text{shape: } (\text{batch}, \text{num head}, \text{seq len}, \text{seq len})$$

b) **Scale**: Normalize by the square root of the head dimension to maintain unit variance:

$$A' \leftarrow \frac{A'}{\sqrt{\text{head dim}}}$$
...
$$A = \text{softmax}(A') \quad \text{shape: } (\text{batch}, \text{num head}, \text{seq len}, \text{seq len})$$
...
$$O' = AV \quad \text{shape: } (\text{batch}, \text{num head}, \text{seq len}, \text{head dim})$$
...
$$
O = O'W_O \quad \text{shape: } (batch, seq len, hid dim)
$$

where $$W_O \in \mathbb{R}^{(\text{num head} \cdot \text{head dim}) \times \text{hid dim}}$$ is a learnable weight matrix.
...
3. Calculate attention scores: $A' = \frac{QK^T}{\sqrt{\text{head dim}}}$
